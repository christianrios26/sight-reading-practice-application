...
public abstract class GameFrame
{
	...
	/**
	 * Draws the silhouette of the provided image, colored using the provided color, at the provided coordinates.
	 * @param image
	 * @param color
	 * @param x
	 * @param y
	 ...
	 * @param height
	 */
	public void drawSolidColorImage(Image image, Color color, int x, int y, int width, int height)
	{
		byte[] redMap = new byte[256];
		byte[] greenMap = new byte[256];
		byte[] blueMap = new byte[256];
		
		for (int i = 0; i < 256; i++)
		{
			redMap[i] = (byte) color.getRed();
			greenMap[i] = (byte) color.getGreen();
			blueMap[i] = (byte) color.getBlue();
		}
		
		byte[][] lookupTable = {redMap, greenMap, blueMap};
		LookupTable table = new ByteLookupTable(0, lookupTable);
		LookupOp lookupOp = new LookupOp(table, null);
		
		BufferedImage alphaImage = new BufferedImage(image.getWidth(null), image.getHeight(null), BufferedImage.TYPE_INT_ARGB);
		alphaImage.getGraphics().drawImage(image, 0, 0, null);
		BufferedImage coloredImage = lookupOp.filter((BufferedImage) alphaImage, null);
		
		graphics2D.drawImage(coloredImage, x, y, width, height, null);
	}
	...
	{
		drawSolidColorImage(image, color, x, y, image.getWidth(null), image.getHeight(null));
	}
	
	/**
	 * Draws the provided image at the provided coordinates.
	 * @param image
	 * @param x
	 * @param y
	 * @param width
	...
	{
		graphics2D.drawImage(image, x, y, image.getWidth(null), image.getHeight(null), null);
	}
